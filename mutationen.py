...
MUTATIONEN = [
    ...
    ('eine stumme Antwort gilt als leere Sessionliste',
     'if not ist_sessionliste(html):', 'if False:'),
    ('ein Konto ohne Sessions gilt als keine Sessionliste',
     'return bool(ID_MUSTER.search(html) or fahrzeugauswahl(html))',
     'return bool(ID_MUSTER.search(html))'),
    ('die Login-Antwort bekommt keine Kopfzeilen im Diagnoseordner',
     "            name='login')", '            name=None)'),
    ('der Rumpf der Login-Antwort wird nicht abgelegt',
     "self._abzug('login.html', rumpf.decode('utf-8', 'replace'))", 'pass'),
    ('Kopfzeilen werden nirgends abgelegt',
     "        if name:\n            self._abzug(name + '.kopfzeilen',",
     "        if False:\n            self._abzug(name + '.kopfzeilen',"),
    ('200 ohne Weiterleitung bleibt eine Nebenbemerkung',
    ...
    ('die Groesse der Sessionliste wird verschwiegen',
     "melde('  Sessionliste: %d Bytes, kein Anmeldeformular' % len(html))",
     'pass'),
    ('die Schlussmeldung empfiehlt, --nur-cache wegzulassen, obwohl es nicht gesetzt war',
     '        if a.nur_cache:\n            raise SystemExit(',
     '        if True:\n            raise SystemExit('),
    ('die leere Sessionliste wird nicht als Beobachtung genannt',
     "    if not ids:\n        melde('  Die Sessionliste ist leer",
     "    if False:\n        melde('  Die Sessionliste ist leer"),
    ('die Abbruchmeldung fasst die Antworten nicht zusammen',
     "% (len(html), self.antworten_zusammenfassen()))",
     "% (len(html), ''))"),
    ('Keks-Werte landen in der Zusammenfassung',
     "kekse.append(wert.partition('=')[0].strip())",
     'kekse.append(wert.strip())'),
    ('fremde Kopfzeilen landen mit Wert in der Zusammenfassung',
     "        elif klein not in nur_namen:\n            nur_namen.append(klein)",
     "        elif klein not in nur_namen:\n"
     "            nur_namen.append('%s: %s' % (klein, wert))"),
    ('der Servername fehlt in der Zusammenfassung',
     "SICHTBARE_KOPFZEILEN = ('Server', 'Content-Type',",
     "SICHTBARE_KOPFZEILEN = ('Content-Type',"),
    ('eine Umleitung der Sessionliste gilt nicht als abgelehnte Anmeldung',
     'if code in UMLEITUNGEN:', 'if False:'),
    ('das Umleitungsziel steht mit Parametern in der Meldung',
     "wohin = urllib.parse.urlsplit(kopfzeilen.get('Location', '')).path",
     "wohin = kopfzeilen.get('Location', '')"),
    ('Feldwerte landen in der Auswertung der Seite',
     "felder.append(name + (' (%s)' % typ if typ in ('hidden', 'password') else ''))",
     'felder.append(tag)'),
    ('der Formularpfad steht mit Parametern in der Auswertung',
     "urllib.parse.urlsplit(attr.get('action') or '').path",
     "(attr.get('action') or '')"),
    ('ein Token-Feld wird nicht erkannt',
     "TOKEN_WORTE = ('csrf', 'token', 'nonce')", 'TOKEN_WORTE = ()'),
    ('ein Captcha wird nicht erkannt',
     "CAPTCHA_WORTE = ('turnstile', 'challenges.cloudflare.com', 'hcaptcha',\n"
     "                 'recaptcha', 'captcha')",
     'CAPTCHA_WORTE = ()'),
    ('fremde Skript-Hosts werden verschwiegen',
     '        if host and host not in hosts:\n            hosts.append(host)',
     '        pass'),
    ('Zeichensalat gilt als HTML',
     "if html and (unlesbar > len(html) / 20 or '<' not in html):",
     'if False:'),
    ('Fehlerwoerter im Text werden nicht gesucht',
     'gefunden = [w for w in FEHLER_WORTE if w in text]', 'gefunden = []'),
    ('die Sitzung aus dem Browser wird nicht mitgeschickt',
     '        self.kekse.update(kekse)', '        pass'),
    ('eine abgelaufene Sitzung wird nicht erkannt',
     'if code in UMLEITUNGEN or \'name="password"\' in html:', 'if False:'),
    ('die Sitzungsdatei wird nicht gelesen',
     '    if os.path.exists(SITZUNG):', '    if False:'),
    ('RACEBOX_SITZUNG wird nicht gelesen',
     "    if os.environ.get('RACEBOX_SITZUNG'):", '    if False:'),
    ('die Sitzung liegt fuer alle lesbar auf der Platte',
     'fd = os.open(SITZUNG, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)',
     'fd = os.open(SITZUNG, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o644)'),
    ('--zugang erzwingt den Weg ueber das Formular nicht',
     'sitzung = None if a.zugang else sitzung_lesen(neu=a.sitzung)',
     'sitzung = sitzung_lesen(neu=a.sitzung)'),
    ('mit Sitzung wird trotzdem das Formular abgeschickt',
     '    if sitzung:\n        html = rb.sitzung_uebernehmen(sitzung)',
     '    if False:\n        html = rb.sitzung_uebernehmen(sitzung)'),
    ('die Captcha-Meldung verschweigt --sitzung',
     "'Sitzung aus dem Browser: rb-golden-lap.py --sitzung\\n'", "'\\n'"),
    ('die Sitzungsfrage nennt ihren Ausgang nicht',
     "'Wert des Kekses racebox (leer = abbrechen): '",
     "'Wert des Kekses racebox: '"),
    ('die Sitzungsfrage sagt nicht, wo der Keks zu finden ist',
     '    melde(SITZUNG_HILFE % SITZUNG)', '    pass'),
    ...
    # -- die Zusammenfassung fuer andere Werkzeuge --
    ('die Zusammenfassung wird nicht geschrieben',
     '    zusammenfassung_schreiben(sessions, cache_ordner)\n', ''),
    ('die Zusammenfassung folgt der Ausblendliste',
     '    strecken, _, paare = strecken_bauen(sessions)',
     '    strecken, _, paare = strecken_bauen(sessions, ausblenden_lesen())'),
    ('ein Eintrag ohne Kennung wird als Session gelesen',
     "        if eintrag.get('id'):\n            raus[eintrag['id']] = eintrag",
     "        raus[eintrag.get('id')] = eintrag"),
    ('Randsektoren werden nicht vermerkt',
     "runden[0]['randsektoren'].append(i + 1)", 'pass'),
    ('die Rundengrenze endet am ersten Punkt',
     '            b[1] = record\n', ''),
    ('die Abweichung je Runde verliert ihr Vorzeichen',
     "'abweichung': round(wanduhr.get(nr, 0.0) - soll, 3),",
     "'abweichung': round(abs(wanduhr.get(nr, 0.0) - soll), 3),"),
    ('RUNDEN_VERSION nicht hochgezaehlt',
     'RUNDEN_VERSION = 4', 'RUNDEN_VERSION = 3'),
    ('abweichende Einteilung gilt als gewertet',
     "status[s['id']] = 'abweichende_einteilung'",
     "status[s['id']] = 'gewertet'"),
    ('Dubletten stehen wie gezaehlte Sessions da',
     '        if sid in dubletten:', '        if False:'),
    ('der Median steht nicht in der Zusammenfassung',
     '            mediane[pos] = median\n', ''),
    ('ein Sektor unter dem halben Median steht ohne Grund da',
     "runde.setdefault('verworfen', {})[pos] = 'unter_halbem_median'",
     'pass'),
    ('ein gerechneter Sektor in einer Teilrunde steht ohne Grund da',
     "verworfen[abgeleitet] = 'gerechnet_in_teilrunde'", 'pass'),
    ('ein alter JSON-Eintrag behauptet, nie gemessen zu haben',
     "if session.get('quelle') == 'json' and 'randsektoren' not in runde:",
     'if False:'),
    ('Sektoren einer unstimmigen Runde zaehlen in der Zusammenfassung',
     "    if not gewertet['stimmig']:\n        return False, 'runde_unstimmig'",
     "    if False:\n        return False, 'runde_unstimmig'"),
    ('Runden aus dem JSON bekommen Grenzen aus der Lap-Spalte',
     "        elif s.get('quelle') == 'json':", '        elif False:'),
    ('jede nicht gezaehlte Runde heisst Teilrunde',
     "            elif gewertet['teilrunde']:", '            elif True:'),
    ('eine verschobene Rundengrenze faellt nicht auf',
     "'auffaellig': (abs(g['abweichung']) > PROBE_GRENZE",
     "'auffaellig': (False"),
    ('der Pfad zum Export ist absolut',
     '        pfad = os.path.relpath(pfad, ordner)',
     '        pfad = os.path.abspath(pfad)'),
    ('verworfene Punkte werden nicht gemeldet',
     "        'verworfene_punkte': verworfen,",
     "        'verworfene_punkte': [],"),
    ('jeder verworfene Punkt ist eine eigene Spanne',
     "if verworfen and verworfen[-1]['bis_index'] == nr - 1:",
     'if False:'),
    ('verworfene Punkte nur bei Runden aus dem Export',
     "                'verworfene_punkte': k.get('verworfene_punkte'),",
     "                'verworfene_punkte': k.get('verworfene_punkte')\n"
     "                if grund is None else None,"),
    ('ein nie gelesener Export gilt als unvollstaendig',
     "        elif k.get('runden_version', 0) < RUNDEN_VERSION:",
     '        elif False:'),
    ('Sektornummer der Zusammenfassung ist die Feldposition',
     "'nummer': (positionen.index(pos) + 1", "'nummer': (pos"),
    ('ohne Cache: Exporte werden nicht uebernommen',
     '    exporte_uebernehmen(cache_ordner)\n', ''),
    ('ohne Cache: ein Eintrag aus dem Netz wird vom Export ersetzt',
     "if alt is None or (alt.get('quelle') == 'export'",
     "if True or (alt.get('quelle') == 'export'"),
    ('ohne Cache: ein uebernommener Export wird bei jedem Lauf gelesen',
     "                           .get('runden_version', 0) < RUNDEN_VERSION):",
     "                           .get('runden_version', 0) <= RUNDEN_VERSION):"),
    ('ohne Cache: eine neue RUNDEN_VERSION liest die Exporte nicht neu',
     "if alt is None or (alt.get('quelle') == 'export'",
     "if alt is None or (False"),
    ('ohne Cache: der Eintrag nennt keine eigene Quelle',
     "eintrag['quelle'] = 'export'", "eintrag['quelle'] = 'csv'"),
    ('ohne Cache: die Zusammenfassung sucht den Export unter kennzahlen',
     "            k = s.get('export') or {}", '            pass'),
    ('ohne Cache: ein Export ohne Datenzeilen gilt als ungelesen',
     "or {'runden_version': RUNDEN_VERSION})", 'or {})'),
    ('ohne Cache: eine Datei ohne RaceBox-Kopf wird Session',
     "if 'RaceBox' not in kopf[:200]:", 'if False:'),
    ('ohne Cache: ein fremder Dateiname wird stillschweigend uebergangen',
     "            fremd.append((name, 'heisst weder <Kennung>.csv noch '\n"
     "                                '<Kennung>_bikemode.csv'))\n",
     ''),
    ('ohne Cache: ein Export namens <Kennung>.csv bleibt fremd',
     "(?:_bikemode)?\\.csv$')", "_bikemode\\.csv$')"),
    ('ohne Cache: die Zusammenfassung findet <Kennung>.csv nicht',
     "EXPORT_NAMEN = ('%s_bikemode.csv', '%s.csv')",
     "EXPORT_NAMEN = ('%s_bikemode.csv',)"),
    ('ohne Cache: <Kennung>.csv geht vor <Kennung>_bikemode.csv',
     "EXPORT_NAMEN = ('%s_bikemode.csv', '%s.csv')",
     "EXPORT_NAMEN = ('%s.csv', '%s_bikemode.csv')"),
    ('ohne Cache: Holen ueberschreibt einen von Hand abgelegten Export',
     '    pfad = os.path.join(ordner, EXPORT_NAMEN[0] % sid)\n    vorlaeufig',
     '    pfad = export_pfad(sid, ordner)\n    vorlaeufig'),
    ('ohne Cache: die Uebersicht haelt Exporte fuer JSON',
     "if s.get('quelle') == 'json'})",
     "if s.get('quelle') != 'json+csv'})"),
    ('ohne Cache: die Meldung verschweigt den Exportordner',
     '% (cache_ordner, CSV_ORDNER))', "% (cache_ordner, '?'))"),
]
